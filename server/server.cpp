...
bool KvServer::process_config() {
  ...
  // shardcontroller
  ShardControllerConfig queried_config = res->config;
  this->config = queried_config;
  ...
  auto all_pairs = this->all_kvpairs();
  for (const auto& [key, value] : all_pairs) {
    auto responsible_server = queried_config.get_server(key);
    if (!responsible_server || *responsible_server != this->address) {
        std::string dest = responsible_server ? *responsible_server : "";
        to_transfer[dest][0].push_back(key);  
        to_transfer[dest][1].push_back(value);
